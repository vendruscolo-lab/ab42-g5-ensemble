...
class CheckBaseline(Callback):
    """Stop training when the model drops below a baseline performance.
    # Arguments
        monitor: quantity to be monitored.
        patience: number of epochs that produced the monitored
            quantity with no improvement after which training will
    ...
    def __init__(self,
                 ...
                 baseline=None,
                 restore_best_weights=False):
        super(CheckBaseline, self).__init__()
            ...
            if self.wait >= self.patience:
                ...
                if self.restore_best_weights:
                    ...
                    if self.best_weights is None:
                        print('Model never reached baseline performance of {0:6.4f}'.format(self.baseline))
                        return
                    self.model.set_weights(self.best_weights)
...
from typing import Tuple, Sequence, List, Union, Generator, Callable, Any, Dict, TypeVar, Set, Optional
...
class DataGenerator:
    ...
    def __call__(self, n: int, lag: int) -> DataSet:
        ...
        # Remove NaNs we put in when we split out the test set
        nan_filter = (np.isfinite(xt).any(axis=1) & np.isfinite(xttau).any(axis=1))
        xt = xt[nan_filter]
        xttau = xttau[nan_filter]
...
class NNModel:
    def __init__(self, model: Union[Model, Layer],
                 ...
                 batchsize: int=10000,
                 epochs: int=100,
                 callback: MaybeListType[Callback]=None,
        ...
        callback
            The callbacks to use while training
...
def _build_model(n_input: int, n_output: int, learning_rate: float=1e-4,
    ...
    models["chi"] = NNModel(
        ...
        callback=[EarlyStopping("val_metric_VAMP", mode="max", min_delta=0.001,
                  ...
                  CheckBaseline("val_metric_VAMP", mode="max", baseline=n_output - np.sqrt(n_output),
...
class KoopmanModel:
    ...
    def K(self) -> np.ndarray:
        ...
        if self.constrained and (self._K is None or self._reestimated):
            self._K = self._models["K"].predict(self.data.trains)[FIRST]
        elif not self.constrained:
            self._K = estimate_koopman(data=self.generator.data, lag=self.lag)
        return self._K
    ...
    def score(self, data: Optional[NNDataType]=None) -> float:
        """
        ...
        """
        data = data if data is not None else self.data.valid
        assert self.chi_estimated
        if self.constrained:
            assert self.aux_estimated
            return -self._models["all"].evaluate(data)
        # TODO: add scoring function
        # return -self._models["chi"].evaluate(data)
